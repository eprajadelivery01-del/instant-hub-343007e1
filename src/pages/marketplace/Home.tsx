  ...
  return (
    <MarketplaceLayout>
        ...
        <div className="max-w-7xl mx-auto flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col">
                 ...
                 <button onClick={() => setAddressSheetOpen(true)} className="flex items-center gap-1 group outline-none">
                    <span className="text-sm font-black text-slate-900 truncate max-w-[150px]">
                       {selectedAddress ? `${selectedAddress.street}, ${selectedAddress.number}` : 'Definir endereço'}
                    </span>
                    <ChevronDown className="h-3 w-3 text-primary animate-bounce-slow" />
                 </button>
              </div>
            </div>
            ...
            <div 
              onClick={() => navigate('/marketplace/profile')}
              className="h-12 w-12 rounded-[18px] bg-white border border-slate-100 shadow-sm flex items-center justify-center overflow-hidden cursor-pointer active:scale-95 transition-all p-1"
            >
              {user?.avatar_url ? (
          ...
          <div className="relative group">
            ...
            <Input
              ...
              className="pl-12 bg-white border border-slate-100 shadow-sm h-12 rounded-[16px] text-sm font-bold placeholder:text-slate-300 focus-visible:ring-2 focus-visible:ring-primary/10"
            />
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 mt-8">
        ...
        <div className="mb-10">
           <div className="flex gap-4 overflow-x-auto scrollbar-hide py-2">
             {categories.map(cat => (
               ...
               >
                 <cat.icon className={cn("h-5 w-5 transition-transform", activeCategory === cat.value ? "scale-110" : "")} />
                 {cat.label}
               </button>
             ))}
           </div>
        ...
        <div className="mb-12 rounded-[40px] overflow-hidden shadow-2xl shadow-black/5 border border-slate-100">
           <HeroMapSection />
        </div>
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-2xl font-black text-slate-900 tracking-tight">O que temos por perto</h2>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-x-4 gap-y-8 pb-32">
