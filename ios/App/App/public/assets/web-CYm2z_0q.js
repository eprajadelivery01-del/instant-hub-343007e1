import{aW as C,aX as M,aY as ae,aZ as se,a_ as ce,a$ as de,b0 as k,b1 as ue,b2 as fe,b3 as F,b4 as le,b5 as q,b6 as T,aV as pe}from"./index-CxrV891J.js";/**
 * @license
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const j="firebase-messaging-database",H=2,l="firebase-messaging-store",f="firebase-messaging-fid-registration-store",Ie={openDB:q,deleteDB:T};let U=Ie,y=null;function _e(e,t,n){switch(t){case 0:if(e.createObjectStore(l),n===1)break;case 1:n===2&&e.createObjectStore(f)}}function L(e){return{upgrade:(t,n)=>{_e(t,n,e)},blocked:()=>{},blocking:(t,n,i)=>{var o;y=null,(o=i.target)==null||o.close()},terminated:()=>{y=null}}}function w(){return y||(y=U.openDB(j,H,L(2)).catch(()=>U.openDB(j,H-1,L(1)))),y}function z(e,t){return e.objectStoreNames.contains(t)}function R(e){if(!z(e,f))throw c.create("fid-registration-idb-schema-unavailable")}async function X(e){const t=g(e),i=await(await w()).transaction(l).objectStore(l).get(t);if(i)return i;{const o=await Te(e.appConfig.senderId);if(o)return await D(e,o),o}}async function D(e,t){const n=g(e),i=await w(),o=[l],r=z(i,f);r&&o.push(f);const a=i.transaction(o,"readwrite");return await a.objectStore(l).put(t,n),r&&await a.objectStore(f).delete(n),await a.done,t}async function Ee(e){const t=g(e),i=(await w()).transaction(l,"readwrite");await i.objectStore(l).delete(t),await i.done}async function A(e){const t=g(e),n=await w();return R(n),await n.transaction(f).objectStore(f).get(t)}async function Re(e,t){const n=g(e),i=await w();R(i);const o=i.transaction([l,f],"readwrite");return await o.objectStore(f).put(t,n),await o.objectStore(l).delete(n),await o.done,t}async function De(e){const t=g(e),n=await w();R(n);const i=n.transaction(f,"readwrite");await i.objectStore(f).delete(t),await i.done}function g({appConfig:e}){return e.appId}const B="@firebase/messaging",E="0.13.1";/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
